        ...
        <div class="card shadow">
            <div class="card-header">
                <div class="d-sm-flex align-items-center justify-content-between mb-4">
                    <h1 class="h3 mb-0 text-gray-800">{{ __('Редактировать результаты тестирования')}}</h1>
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control" name="questions[]" multiple id="question">
                            @foreach($questions as $id => $question)
                                <option
                                    {{ (in_array($id, old('questions', [])) || isset($result) && $result->questions->contains($id)) ? 'selected' : '' }} value="{{ $id }}">{{ $question }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="form-group">
                        ...
                        <input type="number" class="form-control" id="total_points"
                               ...
                               value="{{ old('total_points', $result->total_points) }}"/>
                    </div>
