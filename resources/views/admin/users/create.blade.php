        ...
        <div class="card shadow">
            <div class="card-header">
                <div class="d-sm-flex align-items-center justify-content-between mb-4">
                    <h1 class="h3 mb-0 text-gray-800">{{ __('Добавить пользователя') }}</h1>
            ...
            <div class="card-body">
                <form action="{{ route('admin.users.store') }}" method="POST">
                    ...
                    <div class="form-group">
                        <label for="roles">{{ __('Роли (можно выбрать несколько)') }}</label>
                        <select name="roles[]" id="roles" class="form-control select2" multiple="multiple" required>
                            @foreach($roles as $id => $user_role)
                                <option
                                    value="{{ $id }}" {{ (in_array($id, old('roles', [])) || isset($role) && $role->roles->contains($id)) ? 'selected' : '' }}>{{ $user_role }}</option>
                            @endforeach
                        </select>
                    </div>
